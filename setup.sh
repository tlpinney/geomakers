...
# assuming you've alreay installed git and cloned this repo to your user's home directory with the command on the following line
# git clone http://github.com/geomakers/geomakers_project.git ~/geomakers_project
sudo apt-get update;
sudo apt-get install -y curl vim postgresql libpq-dev postgresql-server-dev-all python python-dev python-pip;
